...
def helper(indices,start,end,result):   #start:{   end:}
    ...
    while(i < end):
        if indices[i]==58:            # ^
            # --- 边界检查: 确保 i+1 在有效范围内 ---
            if i + 1 < end and indices[i+1]==3:
                end1=find_end_bigbracket(indices,i+1,end)
                if end1 == 0: # 如果找不到匹配的 '}'，安全地前进
                    i += 1
                    continue
                if special:
        ...
        #         i=i+1
            ...
            result[i].append(flag[3])
            # --- 边界检查: 确保 i+1 及其后都在有效范围内 ---
            if i + 1 < end:
                end1=find_end_bigbracket(indices,i+1,end)
                if end1 > 0 and end1 + 1 < end: # 确保第一个{}找到，且后面还有空间
                    end2=find_end_bigbracket(indices,end1+1,end)
                    if end2 > 0: # 确保第二个{}也找到了
                        for j in range(i+2, end1):
                            result[j].append(flag[4]) #if indices[j] not in {42,81,82, 83, 110, 112} else None            
                        ...
                        i = end1 + 1
                else: # 找不到第一个{}或后面没空间了，安全地跳过
                    i += 1
            else: # \frac 是最后一个token，安全地跳过
                i += 1
